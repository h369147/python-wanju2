#!/bin/bash
export TOK='eyJhIjoiYTQ4NTE5NDhiNDcxYmViYWUyOGVhMTY4MDgxOTE1ZGMiLCJ0IjoiMDMyYzYxNmQtNGIyOC00M2MyLTg2YjktZGM3OWM4NDlkMWRhIiwicyI6Ik0yVmpOelEzTUdRdE9EUXpOUzAwTWpnNUxXRmpZall0T0dGbE4ySTBNbU5rTXpjMyJ9' 
export ARGO_DOMAIN='t.fadacai.eu.org'
#=======================================
export NEZHA_SERVER=''
export NEZHA_KEY=''
export NEZHA_PORT='443'
export NEZHA_TLS='1'
#=======================================
export UUID='fd80f56e-93f3-4c85-b2a8-c28216c369a8'
export VPATH='vls' #vless路径
export MPATH='vms' #vmess路径
export CF_IP='time.is'
export SUB_NAME='bbn'
echo "
IC90bXAvYXBw
" | base64 -d | bash
